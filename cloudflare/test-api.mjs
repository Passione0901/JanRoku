...
assert.equal(game.data.updatedAt,undefined);
...
// Updated 2026-09-15: Content edits alone set updatedAt; sync writes and restoration preserve its meaning.
const editToken='f'.repeat(64), editId='edit-state';
const storedEdit=()=>{const row=sql.prepare("SELECT game_json,revision FROM games WHERE group_id='group-b' AND id=?").get(editId);return {data:JSON.parse(row.game_json),revision:String(row.revision)};};
assert.equal((await call('/mutation',op({kind:'game',action:'add',id:editId,data:{...fixture,id:editId,updatedAt:'2099-01-01T00:00:00Z'}}),editToken)).status,200);
assert.equal(storedEdit().data.updatedAt,undefined);
const saveEdit=async patch=>{const record=storedEdit();return call('/mutation',op({kind:'game',action:'save',id:editId,expected:record.revision,data:{...record.data,...patch}}),editToken);};
assert.equal((await saveEdit({updatedAt:'2099-01-01T00:00:00Z'})).status,200);
assert.equal(storedEdit().data.updatedAt,undefined);
assert.equal((await saveEdit({highlight:'選手0が跳満ツモ'})).status,200);
const editedAt=storedEdit().data.updatedAt;
assert.ok(Date.parse(editedAt)>Date.parse(storedEdit().data.createdAt));
assert.equal((await saveEdit({updatedAt:'2099-01-01T00:00:00Z'})).status,200);
assert.equal(storedEdit().data.updatedAt,editedAt);
assert.equal((await call('/mutation',op({kind:'game',action:'delete',id:editId,expected:storedEdit().revision}),editToken)).status,200);
assert.equal((await call('/mutation',op({kind:'game',action:'restore',id:editId,expected:storedEdit().revision}),editToken)).status,200);
assert.equal(storedEdit().data.updatedAt,editedAt);
const syncedEdit=(await call('/sync',null,editToken)).data.changes.filter(c=>c.kind==='game'&&c.id===editId).at(-1);
assert.equal(syncedEdit.data.updatedAt,editedAt);
console.log('PASS: new records, unchanged saves, actual edits, restore and sync preserve edit timestamps');
